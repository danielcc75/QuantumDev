...
<div class="w-full">
    <main class="p-4 sm:p-6 lg:p-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6 md:mb-8">
            <div>
                <h2 class="text-2xl md:text-3xl font-bold text-[#1e3a5f]">{{ __('general.proyectos.titulo') }}</h2>
                <p class="text-sm text-gray-500 mt-1">{{ __('general.proyectos.subtitulo') }}</p>
            ...
            <button onclick="abrirModalProyecto()"
                ...
                <i class="fas fa-plus text-xs"></i> {{ __('general.proyectos.btn_nuevo') }}
            </button>
        </div>
        ...
        <div id="empty-state" class="{{ $proyectos->isEmpty() ? '' : 'hidden' }} bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
            ...
            <p class="text-gray-600 font-semibold">{{ __('general.proyectos.vacio_titulo') }}</p>
            <p class="text-xs text-gray-400 mt-1 mb-4">{{ __('general.proyectos.vacio_subtitulo') }}</p>
            <button onclick="abrirModalProyecto()"
                ...
                <i class="fas fa-plus text-xs"></i> {{ __('general.proyectos.vacio_btn') }}
            </button>
        </div>
